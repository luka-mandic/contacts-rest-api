<template>
    <div id="root">
        ...
        <form action="" class="form" style="font-size: 1.2rem">
            ...
            <hr>
            <button v-on:click.prevent="createContact()" class="btn btn-primary mr-3">Create</button>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            createContact() {
                this.authInstance.post('api/contacts', this.contact)
                    .then(response => {
                        this.$router.push('/contact/' + response.data);
                    })
                    .catch(error => {
